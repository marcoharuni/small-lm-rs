...
from nilemini.config import MODEL, artifact_dir, checkpoint_dir, load_sft_profile, workspace_root
from nilemini.generation import chat_prompt
from nilemini.model import initialize_model
from nilemini.tokenizer import load_frozen_tokenizer, tokenizer_sha256
...
def export_model(params_path: Path, *, root: Path, output_dir: Path | None = None) -> Path:
    ...
    save_safetensors(
        ...
        str(weights),
        metadata={"format": "nilemini-v1", "dtype": "float32", "tied_embeddings": "true"},
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Export NileMini parameters for Rust inference")
