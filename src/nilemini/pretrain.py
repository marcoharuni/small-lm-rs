...
from nilemini.data import TokenBatcher, paths_for, prepare_pretraining_data
from nilemini.model import initialize_model
from nilemini.optimizer import build_optimizer, optimizer_group_counts
from nilemini.tokenizer import (
    ensure_tokenizer,
...
def require_gpu() -> None:
    ...
    if not any(device.platform == "gpu" for device in jax.devices()):
        raise RuntimeError("NileMini training requires a JAX GPU runtime")
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Run/resume NileMini pretraining")
