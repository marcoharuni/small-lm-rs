//! Integration checks for scheduler-driven generation on the bundled artifact.
...
fn scheduler_matches_independent_cached_generation() {
    ...
    let expected = prompt_tokens
        ...
        .map(|tokens| {
            ...
            .expect("independent cached generation")
        })
        .collect::<Vec<_>>();
    ...
    while scheduler.active_sequence_count() > 0 {
        for event in scheduler.step(&model).expect("scheduler step") {
            actual
        ...
        sequence_ids.iter().zip(expected.iter()).zip(prompts.iter())
    {
        assert_eq!(
            ...
            "scheduler token sequence differs for prompt {prompt:?}"
        );
        assert_eq!(
